...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S20 at `9488cc88` is current canon and world main; site pin `52fcc69a`, deploy run `31153235627`, and exact public bytes complete custody. The parcel drain's five-case attempt was wholly reverted, so main is not ahead of the blessing; its fresh remainder is nineteen. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
